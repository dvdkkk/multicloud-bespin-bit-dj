...
export default function App() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 즉시 성공 처리 (낙관적 UI: 0.1초 체감 완료)
    setIsSubmitted(true);

    // 백그라운드 전송 (keepalive: true로 페이지 이탈/창 닫기 시에도 전송 보장)
    try {
      fetch('https://inputhaven.com/api/v1/submit', {
        ...
        // CORS 또는 네트워크 이슈 시 no-cors 모드로 fallback 전송
        fetch('https://inputhaven.com/api/v1/submit', {
          method: 'POST',
          body: formData,
          keepalive: true,
  ...
  desc: (
    <>
      ...
      <div className="mt-12 flex justify-center">
        <button 
          onClick={() => scrollTo('contact-form')}
          className="bg-blue-600 text-white hover:bg-blue-700 px-10 py-5 rounded-full font-black text-xl transition-all shadow-2xl hover:scale-105 flex items-center justify-center gap-3 mx-auto"
        >
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-2xl">
              {isSubmitted ? (
                <div className="py-10 md:py-14 flex flex-col items-center text-center">
                  ...
                  <p className="text-slate-600 leading-relaxed max-w-sm mb-8 text-sm md:text-base">
                    신청해주신 정보가 안전하게 접수되었습니다.<br />
                    전문 상담 매니저가 확인 후 빠른 시일 내에 연락드리겠습니다.
                  </p>
                  ...
                  <div className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 mb-8 text-left text-sm text-slate-600 space-y-2">
                    <div className="flex items-center justify-between text-slate-800 font-semibold">
                      <span>상담 안내 시간</span>
                      <span className="text-blue-600 font-bold">평일 09:00 ~ 18:00</span>
                    </div>
                    <p className="text-xs text-slate-400">
                      * 순차적으로 담당자가 유선 및 문자로 상세 일정을 안내해 드립니다.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-all text-sm cursor-pointer"
                  >
                    추가 문의 작성하기
                  </button>
                </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        readOnly
                        className="w-full px-4 py-2 md:py-2.5 rounded-xl border border-slate-200 bg-slate-100/90 font-semibold text-slate-800 outline-none transition-all cursor-default"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        placeholder="궁금하신 내용을 적어주세요. (선택사항)"
                      ></textarea>
                    </div>
